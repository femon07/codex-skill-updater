...
- If precheck reports some `FAIL` rows, the updater still continues for updateable skills and returns exit code `1`.
...
- For `Permission denied (publickey)`, verify SSH auth with `ssh -T git@github.com`; source-map based private skills surface this as `CONFIG_ERROR`.
