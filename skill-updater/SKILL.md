...
- `python3 scripts/update_skills.py --dry-run --allow-manual-map --source-map ./config/skills_source_map.json --jobs 4`

3. Apply updates.
- `python3 scripts/update_skills.py --allow-manual-map --source-map ./config/skills_source_map.json --jobs 4`

4. Debug mode only (fixed file names in current directory).
- `python3 scripts/update_skills.py --dry-run --allow-manual-map --source-map ./config/skills_source_map.json --debug-artifacts`
- outputs: `skill_update_check.debug.tsv`, `skill_update_apply_report.debug.json`
...
- `apply_skill_updates.py` can read check TSV from stdin with `--check-file -`.
