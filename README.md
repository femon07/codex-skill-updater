...
- バックアップとロールバックあり（保存先は常に `$CODEX_HOME/skill-updater-backups/<timestamp>`）
...
- 現行運用のバックアップ先は `~/.codex/skill-updater-backups` のみです。
- 旧運用ディレクトリ（`~/.codex/backups`、`skill-updater/backups`、`codex-skill-updater/backups`）はレガシー扱いです。
- 必要なら旧ディレクトリを新ディレクトリに集約し、最新3世代だけ残して削除してください。
